...
import type { PlaysProps } from "@/app/schedules/lib/types";
    ...
    <Card className={cn("mx-auto w-full max-w-3xl overflow-hidden", className)}>
      <CardHeader>
        <CardTitle className="flex items-baseline justify-between">
          ...
          <span className="text-lg text-right">{price}</span>
        </CardTitle>
        ...
        <div className="flex flex-col gap-4">
          {sessions.map((session, i) => (
            <div key={i}>
              ...
              <p className="text-sm font-medium mb-2">{session.playerLevel}</p>
      ...
      {showDetailButton && (
        <CardFooter className="flex w-full justify-center">
          <Button asChild className="hover:bg-gray-800 rounded-full w-[60%]">
            <RouterLink href={detailHref ?? `/schedules/${id}`}>
              Detail
            </RouterLink>
          </Button>
        </CardFooter>
