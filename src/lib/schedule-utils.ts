import type { PlaySession } from "@/app/schedules/lib/types";
...
export function mergeSessionsByTime(
  ...
  return Object.values(
    sessions.reduce<Record<string, PlaySession>>((acc, session) => {
      ...
      if (existing) {
        const sessionsCourts = Array.isArray(session.court)
          ? session.court
          : [session.court];
        const existingCourts = Array.isArray(existing.court)
          ? existing.court
          : [existing.court];
        ...
        acc[key] = {
          ...session,
          court: Array.isArray(session.court)
            ? [...session.court]
            : [session.court],
        };
      }
