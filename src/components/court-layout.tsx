...
import type {
  HallBlueprintProps,
  RowMeasurement,
  CourtLayout as CourtLayoutType,
  Row,
  RowLayout as RowLayoutType,
  RowOrientation,
...
const layoutRows = (
  ...
  const rows: RowLayoutType[] = measurements.map((measurement, rowIndex) => {
    const { row, courtSize, width, height } = measurement;
    const courts: CourtLayoutType[] = row.courts.map((court, idx) => {
      const xOffset =
        padding +
        (widestRow - width) / 2 +
...
export const CourtLayout: React.FC<HallBlueprintProps> = ({
  hall,
  padding = 32,
    ...
    >
      {laidOutRows.flatMap((row) =>
        row.courts.map((court) => {
          ...
          const stroke = isBooked ? "transparent" : DEFAULT_FILL;
          const strokeWidth = 2;
      ...
      {detailHref ? (
        <CardFooter className="mb-4 mt-2 flex w-full justify-center">
          <Button className="hover:bg-gray-800 rounded-full w-[70%]" asChild>
            <RouterLink href={detailHref}>{detailLabel}</RouterLink>
          </Button>
        </CardFooter>
      ) : null}
    </Card>
