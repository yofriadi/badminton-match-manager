...
export const Schedule: React.FC<PlaysProps> = ({ schedule }) => {
  const { hall, price, date, sessions, tags } = schedule;
  return (
    <Card className="w-full max-w-[calc(100vw)] overflow-hidden px-6 space-y-8">
      <CardHeader>
        <CardTitle className="flex items-baseline justify-between pt-4">
          <span>{hall}</span>
      ...
      <CardFooter className="mb-4 flex w-full justify-center gap-8">
        <AlertDialog>
          <AlertDialogTrigger asChild>
            <Button variant="ghost" className="rounded-full w-24 uppercase">
              cancel
            </Button>
          </AlertDialogTrigger>
          <AlertDialogContent>
        ...
        <Button className="hover:bg-gray-800 rounded-full w-24 uppercase">
          edit
        </Button>
      </CardFooter>
    </Card>
